...
body_classifier = body_classifier.cuda()
...
class MiApp(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        self.ui.pushButton_4.clicked.connect(self.vl_test_vid)
        ...
        self.ui.list_vid_list_view.doubleClicked.connect(self.on_click)
...
#control video player in history
    ...
    def vl_test_vid(self):
            self.ui.stackedWidget_2.setCurrentWidget(self.ui.list_view_test)
    ...
    def on_click(self, index):
            ...
            path =rf'{os.path.dirname(os.path.abspath(__file__))}\test_video\{x}'
            ...
            print(output_path)
            print(json_p)
            car_detect.process_video(path, output_path , json_p )
    ...
    def go_vid(self):
        ...
        if self.txt_combo_2 == "None" or self.txt_combo_3 == "None" or self.del_mp4 == ""  or self.vid_path == "" :
            self.ui.label_8.setText("Invalid inputs")
        else:
            self.ui.label_8.setText(" ")
            self.out_path = rf"{Vpath}\history\detected_{self.del_mp4}.mp4"
            js_path = rf"{Vpath}\json_folders\detected_{self.del_mp4}.json"
            print(self.out_path)
            print(js_path)
            car_detector = video_processing.Detector()
            car_detector.process_video(self.vid_path , self.out_path , js_path)
            #json_file = self.js_path
            #input_video = self.out_path
            output_video = rf"{Vpath}\history\result_{self.del_mp4}.mp4"
            car_colour = self.txt_combo_2
            car_body =  self.txt_combo_3
            car_search( js_path , self.vid_path , output_video , car_colour , car_body )
            self.ui.stackedWidget_3.setCurrentWidget(self.ui.page_vidp_p)
            self.ui.mediaPlayer_sp.setMedia(QMediaContent(QUrl.fromLocalFile(rf"{output_video}")))
...
class VideoThread(QThread):
    ...
    def process_frame(self,frame):
        ...
        transform_body = transforms.Compose([transforms.ToTensor(),
        ...
        for x1, y1, x2, y2, obj_id in track_bbs_ids:
            ...
            if diagonal > 100 :
                try: #TODO optimize
                    ...
                    car_image = transform_color(car_image)
                    car_image = car_image.cuda()

                    body_car_image = frame_model[int(y1):int(y2), int(x1):int(x2)]
                    
                    body_car_image = transform_body(body_car_image)
                ...
                with torch.no_grad():
                    ...
                    body_name = ['Heavy-Duty', 'Lorry', 'Pickup', 'SUV', 'Sedan', 'Van' , '']
...
def car_search(json_file, input_video, output_video='test.mp4', car_colour='', car_body=''):
    ...
    for frame in frame_objects:
        ...
        for obj_id, bbox in frame_objects[frame]:
            ...
            cv2.putText(frame_img, str(object_dict[obj_id]["body_mode_pred"]), (int(bbox[0]), int(bbox[1])), cv2.FONT_HERSHEY_SIMPLEX, 1, (203, 192, 255), 2)
