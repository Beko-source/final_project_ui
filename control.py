...
if not os.path.exists(rf"json_folder"):
            os.mkdir(rf"json_folder")
...
class MiApp(QtWidgets.QMainWindow):
    ...
    def go_vid(self):
        ...
        self.ui.mediaPlayer_sp.setMedia(QMediaContent(QUrl.fromLocalFile(rf"{Vpath}\history\search_result_cars_{self.del_mp4}.mp4")))
...
#control video player in history
  ...
  ####################
    ...
    def open_file(self):
        ...
        print(filename)
        if filename != " ":
            detect_cars = video_processing.Detector()
            time_now = QDateTime.currentDateTime().toString('yyyy-MM-dd-hh-mm')

            print(time_now)
            output_path = rf"{Vpath}\videos\test_video\test-{time_now}.mp4"
            json_p =  rf"{Vpath}\json_folders\test-{time_now}.json"
            print(output_path)
            print(json_p)
            detect_cars.process_video(filename, output_path,json_p )
...
###############################################################
    ...
    def run(self):
        ...
        while True:
            ...
            folder_path_json = os.path.join(parent_dir_json, directory)

            if not os.path.exists(folder_path_json):
                os.mkdir(folder_path_json)
            if not os.path.exists(folder_path_json):
                os.mkdir(folder_path_json)
...
def car_search(json_file ='', input_video='', output_video='',car_colour ='', car_body =''):
    #json_file = open('object_tracks.json')
    open_json = open(json_file)
    object_dict = json.load(open_json)
