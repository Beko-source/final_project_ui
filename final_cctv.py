...
"}")
        ...
        options = ['None' ,'Heavy-Duty', 'Lorry', 'Luxury', 'Pickup', 'SUV', 'Sedan', 'Van']
...
"\n"
        ...
        # Hide all files except for those specified in setNameFilters
        # Hide all files that do not match the specified extensions
        for i in range(self.h_file_system_model.rowCount()):
                index = self.h_file_system_model.index(i, 0)
                fileInfo = self.h_file_system_model.fileInfo(index)
                if fileInfo.isFile() and not fileInfo.suffix() in ['mp4', 'avi', 'mkv']:
                        self.h_file_system_model.setData(index, QtCore.QVariant(), QtCore.Qt.DisplayRole)       
